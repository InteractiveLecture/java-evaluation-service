...
package org.lecture.compiler.compiler;
...
public class StringCompiler 
{
    ...
    public CompilationResult startCompilation(ClassLoader cl)
    {
        ...
        JavaFileManagerProxy jfm = new JavaFileManagerProxy(compiler.getStandardFileManager(diagnostics, null, null));
        compiler.getTask(null, jfm, diagnostics, null, null, javaFileObjects).call();
        diagnostics.getDiagnostics().stream().forEach((diagnostic)-> result.addDiagnostic(diagnostic));
        try
        {
            jfm.close();
        }
        ...
        jfm.getCompiledBytes().forEach((classname,bytes)-> {
            if(bytes != null && bytes.length > 0)
                result.getCompiledClasses().put(classname,this.defineClass(result.getClassLoader(), classname, bytes));
        });
    ...
    private Class<?> defineClass( ClassLoader classLoader,  String className,  byte[] bytes) 
    {
        ...
        try 
        {
            ...
            result = (Class<?>) m.invoke(classLoader, className, bytes, 0, bytes.length);
